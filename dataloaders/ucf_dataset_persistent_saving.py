...
if __name__ == '__main__':
    ...
    for i_batch, (images, targets) in enumerate(test_dataloader):
        result_dir = "/workspaces/video-dataset-preprocess/Dataset/UCF-results"
        os.makedirs(result_dir, exist_ok=True)

        # Loop over the time dimension
        for time_step in range(images.shape[1]):
            # Select a specific time step
            image_at_timestep = images[i_batch][time_step]

            # Convert the tensor to numpy array and transpose dimensions
            numpy_image = image_at_timestep.permute(1, 2, 0).numpy()

            # If the image is in RGB format, convert it to BGR
            if numpy_image.shape[2] == 3:
                numpy_image = cv2.cvtColor(numpy_image, cv2.COLOR_RGB2BGR)
            
            # Convert the data type and scale the values if necessary
            ...
            # Loop over the channels dimension
            for channel in range(numpy_image.shape[2]):
                # Select a specific channel
                image_at_channel = numpy_image[:, :, channel]

                # Create a filename for each frame
                frame_filename = f"{result_dir}/image_{i_batch}_frame_{time_step}_channel_{channel}.jpg"
                os.makedirs(os.path.dirname(frame_filename), exist_ok=True)

                # Print the shape and the number of channels of the image
                # print("Shape of numpy_image:", numpy_image.shape)
                # print("Number of channels in numpy_image:", numpy_image.shape[2])

                # Save the image
                cv2.imwrite(frame_filename, image_at_channel)

            # # Create a filename for each frame
            # frame_filename = f"{result_dir}/image_{i_batch}_frame_{time_step}.jpg"
            ...
            # # Save the image
            # cv2.imwrite(frame_filename, numpy_image)
